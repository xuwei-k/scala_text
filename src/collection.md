...
## [Array](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/Array.scala)（★★）
...
### [Range](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/Range.scala)（★★）
...
### [List](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/List.scala)（★★★）
...
まず最初に紹介するのは`Nil`です。Scalaで空の`List`を表すには`Nil`というものを使います。Rubyなどでは`nil`は言語上かなり特別な意味を持ちますが、Scalaではデフォルトでスコープに入っているということ以外は特別な意味はなく[単にobjectです](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/List.scala#L417)。Nilは単体では意味がありませんが、次に説明する`::`と合わせて用いることが多いです。

### [:: - Listの先頭に要素をくっつける](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/List.scala#L111)
...
### [Vector](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/Vector.scala)（★★★）
...
### [`scala.collection.immutable.Map`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/Map.scala) 

Scalaで何も設定せずにただ`Map`と書いた場合、`scala.collection.immutable.Map`が使われます。その名の通り、一度
作成すると変更することはできません。内部の実装としては主に[`scala.collection.immutable.HashMap`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/HashMap.scala)と
[`scala.collection.immutable.TreeMap`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/TreeMap.scala)の2種類がありますが、通常は`HashMap`が使われます。
...
同じく、一度作成すると変更することはできません。内部の実装としては、主に [`scala.collection.immutable.HashSet`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/HashSet.scala) と
[`scala.collection.immutable.TreeSet`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/immutable/TreeSet.scala) の2種類がありますが、通常は`HashSet`が使われます。
...
Scalaの変更可能な`Set`は`scala.collection.mutable.Set`にあります。主な実装としては、[`scala.collection.mutable.HashSet`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/mutable/HashSet.scala) 、
[`scala.collection.mutable.TreeSet`](https://github.com/scala/scala/blob/v{{ book.scala_version }}/src/library/scala/collection/mutable/TreeSet.scala)がありますが、通常は`HashSet`が使われます。
